...
public class ELGraph<V, E> implements Graph<V, E>{
    ...
    public Collection<? extends Edge<E>> incidentEdges(Vertex<V> v) {
        HashSet<Edge<E>> incidentEdges = new HashSet<>(); // we create a set because we cannot have nodes repetition
    ...
    public List<Vertex<V>> endVertices(Edge<E> edge) {
        ELEdge<E,V> evelEdge = checkEdge(edge);
        ArrayList <Vertex<V> > output = new ArrayList<>();
        output.add(evelEdge.getStartVertex());
        output.add(evelEdge.getEndVertex());
        return output;
    }
    ...
    public Edge<E> insertEdge(Vertex<V> v1, Vertex<V> v2, E edgeValue) {
        ...
        ELEdge<E,V> e = new ELEdge<>(edgeValue,checkVertex(v1),checkVertex(v2),this); // create the edge

        if (edgeList.contains(e))
            edgeList.remove(e);
        edgeList.add(e); // update the edge cost.
        return e;
    }
